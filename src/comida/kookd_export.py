...
from comida.basket import get_basket_for_list
...
def _exclusion_reason(product: dict, force_include: set[str], force_exclude: set[str]) -> str | None:
    blob = _name_blob(product)
...
def _to_kookd_label(product: dict) -> str:
    ...
    if canned_tomato:
        ...
        label = translate_product_name(name)
...
def export_list_to_kookd(
    ...
    *,
    ...
    for product in products:
        ...
        reason = _exclusion_reason(product, force_include, force_exclude)
        if reason:
...
def run_promos_export(
    ...
    *,
    ...
    print(f"Liste Migros : {list_name}")
